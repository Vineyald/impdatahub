...
const InactiveClientsChart = () => {
  ...
  const HeatmapChart = ({ data, title }) => {
    const [dimensions, setDimensions] = useState({ width: 400, height: 300 }); // Tamanho inicial
    const containerRef = useResizeObserver((rect) => {
      // Evitar pequenos ajustes de tamanho
      ...
      <div ref={containerRef} style={{ width: '100%', height: '100%' }}>
        <Plot
          ...
          layout={{
            ...
            autosize: true,
            width: dimensions.width,  // Usar dimensões controladas
  ...
  const BarChart = ({ data, title }) => {
    ...
    <div ref={containerRef} style={{ width: '100%', height: '100%' }}>
       <Plot
          data={[
            {
              x: ['30_60', '60_90', '90_120', 'mais_120'],
              y: data,
              type: 'bar',
              marker: {
                color: ['#202020', '#404040', '#606060', '#808080'],
              },
            }
          ]}
          layout={{
  ...
  return (
    <div className="container">
      ...
      <div className="row">
        <div className="col-6">
          <HeatmapChart data={[countInactiveClientsByMonth(serviData['30_60']), countInactiveClientsByMonth(serviData['60_90']), countInactiveClientsByMonth(serviData['90_120']), countInactiveClientsByMonth(serviData['mais_120'])]} title="Heatmap Inativos - Servi" />
        </div>
        <div className="col-6">
          <HeatmapChart data={[countInactiveClientsByMonth(impData['30_60']), countInactiveClientsByMonth(impData['60_90']), countInactiveClientsByMonth(impData['90_120']), countInactiveClientsByMonth(impData['mais_120'])]} title="Heatmap Inativos - Imp" />
        </div>
      </div>
    </div>
  );
